...
const DEPARTMENTS = ['Eye', 'Dental', 'General', 'Orthopedic', 'Other']
...
const DoctorSignup = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!name.trim() || !email.trim() || !password.trim() || !confirmPassword.trim() || !finalDepartment) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 bg-[#f5f9ff] dark:bg-[#0f172a] transition-colors duration-300">
      <div className="w-full max-w-md p-8 bg-white dark:bg-[#1e293b] rounded-[12px] shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:shadow-none border-0 transition-colors duration-300 my-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Doctor Registration</h1>
          <p className="text-slate-500 dark:text-slate-400">Join the platform to manage appointments</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          {department === 'Other' && (
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" htmlFor="customDepartment">
                Specify Department
              </label>
              <input
                id="customDepartment"
          ...
          <button
            type="submit"
            className="w-full py-3 bg-primary hover:bg-primary-hover text-white font-semibold rounded-[10px] shadow-md hover:shadow-lg transition-all disabled:opacity-70 disabled:cursor-not-allowed mt-2"
        ...
        <p className="mt-8 text-center text-sm text-slate-500 dark:text-slate-400">
          Already registered?{' '}
          <Link to="/doctor-login" className="font-semibold text-primary hover:text-primary-hover transition-colors">
            Log in
          </Link>
        </p>
      </div>
